...
Compile-time IL weaver for .NET. Write normal C# weave templates with `[Weave]` and `[AsyncWeave]`; SharpWeaver splices them into target methods after `CoreCompile` using [Mono.Cecil](https://github.com/jbevain/cecil).
...
| Project | Role |
| :------ | :--- |
| `SharpWeaver.Attributes` | `[Weave]`, `[AsyncWeave]`, capture attributes, `WeaveTemplate` markers |
...
For more examples, see **[SharpWeaver.Examples](#examples)** and the [unit tests / fixtures](SharpWeaver.Tests/) (`ValidPatches.cs`, `ValidAsyncPatches.cs`).
...
`priority` is a **required** constructor argument on `[Weave]` and `[AsyncWeave]`.
...
| Build error: missing priority | Every `[Weave]` / `[AsyncWeave]` must include `priority: N` |
...
面向 .NET 的编译期 IL 织入器。用 `[Weave]` 与 `[AsyncWeave]` 编写普通 C# 织入模板；SharpWeaver 在 `CoreCompile` 之后借助 [Mono.Cecil](https://github.com/jbevain/cecil) 将其拼接到目标方法。
...
| 项目 | 作用 |
| :--- | :--- |
| `SharpWeaver.Attributes` | `[Weave]`、`[AsyncWeave]`、捕获属性、`WeaveTemplate` 标记 |
...
`priority` 是 `[Weave]` 与 `[AsyncWeave]` 构造函数的**必填**参数。
...
| 构建错误：缺少 priority | 每个 `[Weave]` / `[AsyncWeave]` 都必须写 `priority: N` |
